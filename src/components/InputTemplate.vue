...
<template>
  <label class="mt-8 text-14 font-['Jost'] font-medium text-[#919B9B]" :for="formInput.name">{{
    formInput.label
  }}</label>
  <template v-if="formInput.type === 'text'">
    ...
    >
      <Field v-slot="{ meta, field }" :name="formInput.name"
        ><input
          v-bind="field"
          :type="formInput.type"
          class="relative h-[5rem] w-full rounded-full px-20 text-16 [&.is-invalid]:text-[#F41A0C] focus:outline-none focus:ring-0"
          :class="{
    ...
    <ErrorMessage
      :name="formInput.name"
      class="mt-2 self-end text-[#F41A0C] text-10 font-normal"
    />
  </template>
  ...
  <template v-else-if="formInput.type === 'password'">
    ...
    <ErrorMessage
      :name="formInput.name"
      class="mt-2 self-end text-[rgb(244,26,12)] text-10 font-normal"
    />
  </template>
  ...
  <template v-else-if="formInput.type === 'select'">
    ...
    >
      <Field v-slot="{ meta, field }" :name="formInput.name"
        ><select
          v-bind="field"
          class="appearance-none text-16 text-[#919B9B] font-['Jost'] font-medium h-[5.5rem] w-full rounded-full px-20 border-2 border-[#E1E1E1] [&.is-invalid]:border-[#F41A0C] [&.is-invalid]:text-[#F41A0C] after:content-['eee']"
          :class="{
            'is-valid': meta.valid && meta.touched,
            'is-invalid': !meta.valid && meta.touched
          }"
  ...
  <template v-else-if="formInput.type === 'radio'">
    <!-- <label class="text-14 text-[#454238] font-['Gotham'] mt-8">{{ formInput.label }}</label> -->
    <div class="flex gap-4 mt-4">
      <div v-for="(option, index) in formInput.options">
        ...
        <label
          :for="option.id"
          class="px-8 py-4 flex border-gray-350 border-2 rounded-full cursor-pointer text-14 text-[#919B9B] font-['Jost'] font-medium peer-checked:bg-gradient-to-r from-[#1A47BF] to-[#D63ACA] peer-checked:text-white peer-checked:px-[2.2rem] peer-checked:py-[1.2rem] peer-checked:border-none"
          >{{ option.text }}</label
        >
      </div>
    </div>
  ...
  <template v-else-if="formInput.type === 'multicheckbox'">
    ...
    <div class="flex flex-wrap gap-4 mt-4">
      <div v-for="(option, index) in formInput.options">
        ...
        <label
          :for="option.id"
          :class="`px-8 py-4 flex min-w-fit flex-nowrap border-gray-350 border-2 rounded-full cursor-pointer text-16 text-[#919B9B] font-['Jost'] font-medium peer-checked:bg-gradient-to-r from-[#1A47BF] to-[#D63ACA] peer-checked:text-white peer-checked:px-[2.2rem] peer-checked:py-[1.16rem] peer-checked:border-none`"
          >{{ option.text }}</label
        >
      </div>
    </div>
  ...
  <template v-else-if="formInput.type === 'checkbox'">
    <div class="flex flex-nowrap gap-4 mt-4 align-top">
      <template v-for="(option, index) in formInput.options">
        ...
        <label
          :for="option.id"
          class="flex min-w-fit flex-nowrap cursor-pointer text-14 text-[#68838E] font-['Jost']"
          >{{ option.text }}</label
        >
      </template>
    </div>
